<div class="teamleader-container" id="teamleaderContainer<?php echo esc_attr($id); ?>">
	<form id="tmForm<?php echo esc_attr($id); ?>">
		...
		<?php if (true === $options['logo']) : ?>
			<div class="teamleader-powered"><?php _e('Powered by', 'teamleader'); ?>
				<a href="http://referral.teamleader.eu/en?token=<?php echo (!empty($options['referral_token'])) ? esc_attr($options['referral_token']) : 'refferal'; ?>" target="_blank">Teamleader</a>
				<img src="<?php echo esc_url($logo); ?>" alt="<?php _e('Teamleader', $key); ?>">
			</div>
		<?php endif; ?>
		...
		<?php endif; ?>
		<button type="submit" class="btn btn-primary teamleader-submit"><?php echo esc_html($form['form']['submit']); ?></button>
	</form>
	<div class="teamleader-success" data-success style="display: none;"><?php echo esc_html($form['form']['success']); ?></div>
