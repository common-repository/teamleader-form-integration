...
class FrontendController extends AbstractController implements HooksInterface
{
	...
	protected function loadAssets()
	{
		...
		wp_enqueue_script("frontend", Container::pluginUrl() . 'assets/js/frontend.js', array('jquery'), Container::version());


		wp_add_inline_script('frontend', 'const VARS = ' . json_encode(array(
			"id" => $this->id,
			"url" => admin_url('admin-ajax.php'),
			"nonce" => wp_create_nonce('teamleader'),
		)), 'before');
